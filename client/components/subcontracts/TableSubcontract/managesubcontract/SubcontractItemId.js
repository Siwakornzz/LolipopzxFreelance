...
import { UPDATE_SUBCONTRACT } from "../../../../apollo/mutations";
import {
  Me, 
  QUERY_SUBCONTRACT,
  QUERY_SUBCONTRACTS,
...
const SubcontractItemId = () => {
  ...
  console.log("subcontractDat", subcontractData);

  if (!user) {
    return <p class="text-center mt-5"> ไม่ได้เข้าสู่ระบบโปรดเข้าสู่ระบบ</p>;
  }
  const { loading, error } = useQuery(QUERY_SUBCONTRACT, {
    variables: { id: route.query.managesubcontractsItem },
    onCompleted: (data) => {
      setSubcontract(data.subcontract);
      setSubcontractData(data.subcontract);
    },
  });

  const [updateSubcontract] = useMutation(UPDATE_SUBCONTRACT, {
    onCompleted: (data) => {
  ...
  const handleSubmit = async (e) => {
    if (subcontractData === subcontract) {
      ...
      Swal.fire("LOLIPOPZ", "ไม่มีอะไรเปลี่ยนแปลง !", "info");
      return;
    }
    try {
      e.preventDefault();
      ...
      Swal.fire({
        icon: "success",
        title: "LOLIPOPZ",
        text: "อัพเดทข้อมูลงานที่รับทำเสร็จสิ้น",
      });
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-3">
              ...
              <select
                ...
                id="typeofwork"
                name="typeofwork"
                value={subcontractData.typeofwork}
                onChange={handleChange}
                required
              >
            ...
            <div className="text-end">
              <button
                type="submit"
                className="btn btn-warning w-25 text-center mt-3"
              >
                {" "}
